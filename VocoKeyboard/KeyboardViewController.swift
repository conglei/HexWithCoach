...
final class KeyboardViewController: KeyboardInputViewController {
    ...
    private func startSessionBounce() {
        guard let url = URL(string: "voco://startSession") else { return }
        hexState.statusText = "Starting session in Voco…"
        guard let application = firstUIApplicationInResponderChain() else {
            hexState.errorMessage = "Couldn't reach Voco. Open the app once, then try again."
            hexState.statusText = "Couldn't reach the app (no UIApplication in chain)."
            return
        }
        application.open(url, options: [:]) { [weak self] success in
            if !success {
                self?.hexState.errorMessage = "iOS blocked opening Voco."
                self?.hexState.statusText = "iOS blocked opening Voco."
            }
        }
    }
    ...
    private func openHostApp(path: String) {
        guard hasFullAccess, let url = URL(string: "voco://\(path)") else { return }
