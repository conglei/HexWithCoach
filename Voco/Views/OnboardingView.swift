...
struct OnboardingView: View {
    ...
    private func title(_ step: Step) -> String {
        switch step {
        case .keyboard: "Set up the Voco keyboard"
    ...
    private func detail(_ step: Step) -> String {
        switch step {
        case .keyboard:
            "In Settings ▸ Keyboards, add Voco and turn on Allow Full Access. Then tap the box below and switch to the Voco keyboard (🌐) — it'll confirm here automatically."
        case .microphone:
            switch micPermission {
            case .denied: "Access was denied. Enable it in Settings to dictate."
            default: "Voco transcribes entirely on-device — your audio never leaves the phone."
            }
        case .model:
    ...
    private func actions(_ step: Step) -> some View {
        ...
        case .keyboard:
            VStack(spacing: 10) {
                ...
                TextField("Tap here, then switch to the Voco keyboard", text: $confirmText)
    ...
    private var allSetCard: some View {
        VStack(spacing: 16) {
            ...
            Text("Dictate a note from Home, or switch to the Voco keyboard in any app and start a session.")
                .font(.callout).foregroundStyle(.secondary).multilineTextAlignment(.center)
...
#Preview {
    let container = try! ModelContainer(
        for: TranscriptEntry.self, TranscriptAnalysis.self,
