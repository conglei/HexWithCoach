...
final class DictationModel {
    ...
    private func save(text: String, kind: TranscriptKind, audioURL: URL?, words: [VocoCore.WordTiming]? = nil, surface: Bool = true) {
    ...
    /// via `voco://` doesn't kick off a second model load.
    func prepare() async {
    ...
    /// Entry point for the keyboard's session bounce (`voco://startSession`):
